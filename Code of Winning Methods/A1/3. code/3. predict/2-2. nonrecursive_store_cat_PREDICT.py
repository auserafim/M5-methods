...
for cv in cvs:
    ...
    for store in STORES:
        for state in CATS:
            ...
            grid_1 = pd.read_pickle(processed_data_dir+"grid_part_1.pkl")
            pd.concat([grid_1.iloc[indice], prediction], axis=1)            .pivot(index='id', columns='d', values='y_pred')            .reset_index()            .set_index('id')            .to_csv(log_dir+f'submission_storeandcat_{store}_{state}_{cv}.csv')
