...
for cv in cvs:
    ...
    for store in STORES:
        ...
        model_path = model_dir+'non_recur_model_'+store+'.bin'
        ...
        grid_1 = pd.read_pickle(processed_data_dir+"grid_part_1.pkl")
        pd.concat([grid_1.iloc[indice], prediction], axis=1)        .pivot(index='id', columns='d', values='y_pred')        .reset_index()        .set_index('id')        .to_csv(log_dir+f'submission_onlystore_{store}_{cv}.csv')
